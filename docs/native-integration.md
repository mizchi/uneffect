...
The workflow separates unit/build/package checks, Z3 obligations, Quint safety simulations with negative controls, and exhaustive Apalache verification. The exhaustive tier installs Java explicitly; local environments without Java still run every other tier. Hoare, ownership, temporal semantic/reachability/counterexample, property-generation, and typed-array SMT-LIB checks can use an optional native Z3 process and otherwise fall back to `z3-solver` WASM. CI omits native Z3 to retain fallback coverage. The JVM appears only in the deliberately redundant Apalache and TLC checks described in [formal models](./formal-models.md).
