...
shape (`src/support/diagnostics.ts`): a code, a severity, a location, the function under
...
invariant IR that produced the query (`src/contracts/contract-explanations.ts`), with exact
...
`src/support/diagnostic-quality.ts` scores every diagnostic the corpus produces against
