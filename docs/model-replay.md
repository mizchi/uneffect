...
Action dispatch uses only the adapter action table's own properties. An
unregistered `toString` or `constructor` must be reported as `missing-action`,
even when the trace step leaves the observed state unchanged. Those names
remain valid when explicitly registered. Inherited handlers should be copied
into the action table to register them.

TLC action recovery compares normalized Set, Map, and record values by
contents, using the same canonical comparison as state matching. JavaScript
object identity is not a temporal equality test; both equal and unequal
collection guards have regression fixtures.

The standalone JavaScript generated by `generateRuntimeAssertionExpression` /
`generateRuntimeAssertionStatement` also compares finite, acyclic Set, Map, and
record values by contents for equality/inequality. It evaluates each operand
once and recursively compares nested values. This guarantee concerns equality;
it does not establish value semantics for every other generated Set operation.
