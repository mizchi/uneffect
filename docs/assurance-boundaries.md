...
| Hoare contracts | Individual emitted obligations whose artifact status is `verified`. `uneffect-refinement-action-analysis/v2` binds source digest, TypeScript version, and loop/try spans. It retains the ranking-loop certificate checks from v1 and adds one integer environment handed exactly across two source-keyed sibling handler regions. The handler value obligation remains `unknown` after structural convergence; only `analyzeRefinementActionBodiesWithZ3` can upgrade it by independently proving the emitted final expression equivalent to the declared action. Solver failure downgrades the obligation to `unknown`. | The accepted recurrence and two-region shapes are still selected by bounded lowering. Multiple changed states, arbitrary heap aliasing, general recurrence fixed points or widening, arbitrary region counts, irreducible loops, Program/external-action support for these first CFG artifacts, and unsupported expressions remain outside the claim. |
