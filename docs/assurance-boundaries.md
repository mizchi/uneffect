...
| Hoare contracts | Individual emitted obligations whose artifact status is `verified`. `uneffect-refinement-action-analysis/v2` binds source digest, TypeScript version, and loop/try spans. It retains the ranking-loop certificate checks from v1, adds a one- or two-integer product environment handed exactly across bounded source-keyed handler regions, and admits one direct source-bound affine `while` back edge with at most two integer members plus one optional loop-invariant Boolean diamond. Structural convergence remains `unknown`; only `analyzeRefinementActionBodiesWithZ3` can upgrade the value or recurrence after independently checking every member and ranking obligation. Predicate/predecessor source correlation is structural, while Z3 checks the emitted piecewise recurrence equations. Solver failure downgrades the whole product to `unknown`. | The accepted recurrence, handler topology, and direct loop/diamond are still selected by bounded lowering. Predicate loss or mutation, a third changed state, a fourth linear region, arbitrary heap aliasing, cross-member recurrences, self-amplification, general recurrence widening, nested/multiple branch fan-out, nested or irreducible loops, Program/external-action support for these first CFG artifacts, and unsupported expressions remain outside the claim. |
