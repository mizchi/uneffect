...
| Hoare contracts | Individual emitted obligations whose artifact status is `verified`. `uneffect-refinement-action-analysis/v1` binds source digest, TypeScript version, and loop/try spans, then verifies one direct ranking-loop normal/typed-throw catch join only when its named CFG worklist converges, retains the normalized payload, creates the condition-correlated predecessor phi environment, and stabilizes the affine ranking/transformer/summary certificate with no action diagnostic. Consumers requiring defense against a faulty closed-form lowering use `analyzeRefinementActionBodiesWithZ3`, which independently reparses that certificate and proves base, per-state inductive step, and ranking obligations; solver failure downgrades the obligation to `unknown`. | The accepted recurrence shape is still selected by the shape-specific affine lowering. Termination in general, arbitrary heap aliasing, general recurrence fixed points or widening, arbitrary AST-to-basic-block lowering, irreducible loops, Program/external-action support for the first CFG artifact, and unsupported expressions remain outside the claim. |
