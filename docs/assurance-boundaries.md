...
| Hoare contracts | Individual emitted obligations whose artifact status is `verified`. `uneffect-refinement-action-analysis/v2` binds source digest, TypeScript version, and loop/try spans. It retains the ranking-loop certificate checks from v1, adds a one- or two-integer product environment handed exactly across two or three linear source-keyed sibling handler regions or one exact conditional join, and admits one direct source-bound affine `while` back edge with at most two integer members. Structural convergence remains `unknown`; only `analyzeRefinementActionBodiesWithZ3` can upgrade the value or recurrence after independently checking every member and ranking obligation. Solver failure downgrades the whole product to `unknown`. | The accepted recurrence, linear region, conditional topology, and direct loop are still selected by bounded lowering. Predicate loss, a third changed state, a fourth linear region, arbitrary heap aliasing, cross-member recurrences, self-amplification, path-dependent deltas, general recurrence widening, arbitrary branch fan-out/region counts, nested or irreducible loops, Program/external-action support for these first CFG artifacts, and unsupported expressions remain outside the claim. |
