...
| Hoare contracts | Individual emitted obligations whose artifact status is `verified`. `uneffect-refinement-action-analysis/v2` binds source digest, TypeScript version, and loop/try spans. Direct and the one supported handler-backed affine loop share `scalar-recurrence-fixed-point`; the handler form adds source-bound predecessor/payload/snapshot/finally evidence and retains an eight-member compatibility cap, while ordinary direct loops admit at most two integer members plus either at most two sequential, distinct, loop-invariant Boolean diamonds or one unchanged-integer `switch` with two non-negative numeric-literal cases, explicit default, and no fallthrough. Structural convergence remains `unknown`; only `analyzeRefinementActionBodiesWithZ3` can upgrade the value or recurrence after independently checking every member and ranking obligation. Predicate/case/predecessor/order source correlation is structural, while Z3 checks the emitted composed recurrence equations. Solver failure downgrades the whole product to `unknown`. | The accepted recurrence, handler topology, and direct loop joins are still selected by bounded lowering. The eight-member handler cap is compatibility capacity, not general product widening. Predicate reuse, loss, or mutation, discriminant mutation, fallthrough, signed/dynamic/excess cases, a third direct-loop changed state, a third diamond, a fourth linear region, arbitrary heap aliasing, cross-member recurrences, self-amplification, general recurrence widening, nested branch fan-out, nested or irreducible loops, Program/external-action support for these first CFG artifacts, and unsupported expressions remain outside the claim. |
