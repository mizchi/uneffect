...
| ESM initialization order | A `uneffect-module-order/v1` artifact with `evidence: "verified"`, for its represented acyclic local graph or synchronous side-effect-import simple ring. Every edge retains source span/digest and semantic rule; the artifact binds TypeScript/compiler-options identity. | Runtime-binding, self, branching, multi-edge, or asynchronous cycles; sibling initiation during suspension; conditional TLA/throw; decorators/classes; dynamic/external bodies; deployment scheduling; or proof that normal completion occurs. |
