...
| Hoare contracts | Individual emitted obligations whose artifact status is `verified`. `uneffect-refinement-action-analysis/v2` binds source digest, TypeScript version, and loop/try spans. Direct and the one supported handler-backed affine loop share `scalar-recurrence-fixed-point`; the handler form adds source-bound predecessor/payload/snapshot/finally evidence and retains an eight-member compatibility cap, while ordinary direct loops admit at most two integer members plus one optional loop-invariant Boolean diamond. Structural convergence remains `unknown`; only `analyzeRefinementActionBodiesWithZ3` can upgrade the value or recurrence after independently checking every member and ranking obligation. Predicate/predecessor source correlation is structural, while Z3 checks the emitted recurrence equations. Solver failure downgrades the whole product to `unknown`. | The accepted recurrence, handler topology, and direct loop/diamond are still selected by bounded lowering. The eight-member handler cap is compatibility capacity, not general product widening. Predicate loss or mutation, a third direct-loop changed state, a fourth linear region, arbitrary heap aliasing, cross-member recurrences, self-amplification, general recurrence widening, nested/multiple branch fan-out, nested or irreducible loops, Program/external-action support for these first CFG artifacts, and unsupported expressions remain outside the claim. |
