...
| Native frontend parity | `semanticEquivalent: true` establishes semantic agreement for the emitted neutral-IR fields. An in-process exporter result plus `requireCorsaCheckerFacts` establishes that supported top-level function declarations, single immutable arrow/function-expression bindings, identifier-named methods of top-level classes, named-function overload candidates/selections, type/trivia, direct intra-project calls, reviewed builtin effect atoms, direct unconditional `await` observations, and direct awaits beneath exactly one enclosing `if` branch came from full Corsa parser services. Single-if evidence includes owner-local file-offset condition identity, polarity, and one control path; owner plus project-wide operation span distinguish cross-file records. Overload/effect/await metadata is compared separately because normalized effect projection omits some evidence. | Computed or polymorphically dispatched methods, callback timing, method/generic overload edge cases, nested/loop/catch conditional Promise observations, return-observation records, rejection ownership, resource facts, and other neutral-IR domains are not checker-exported. Explicitly annotated computed methods and unsupported conditional awaits make comparison fail coverage/evidence rather than being flattened. Calls or awaits inside unsupported nested functions are omitted rather than presented as immediate outer work. Cloned/persisted facts are unauthenticated, and there is no signed evidence format or whole-program Corsa parity. |
