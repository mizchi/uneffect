...
| Temporal logic | A generated artifact plus the reported backend result and bounds/fairness assumptions. Bounded dynamic Map traces may use multiple direct scalar keys only after every key receives separate finite-domain initiation and preservation proofs. | An unbounded proof when only simulation or bounded checking ran; compound keys, partial domain proofs, and general collection correlations. |
