...
| Native frontend parity | `semanticEquivalent: true` establishes semantic agreement for the emitted neutral-IR fields. An in-process exporter result plus `requireCorsaCheckerFacts` establishes that supported top-level function/type/trivia and direct intra-project call facts came from full Corsa parser services, including cross-file and duplicate-spelling symbol identity. | Methods, arrows, callbacks/timing, overloads, Promise/resource facts, and other neutral-IR domains are not checker-exported. Cloned/persisted facts are unauthenticated, and there is no signed evidence format or whole-program Corsa parity. |
