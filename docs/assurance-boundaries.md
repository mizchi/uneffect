...
| Hoare contracts | Individual emitted obligations whose artifact status is `verified`. `uneffect-refinement-action-analysis/v2` binds source digest, TypeScript version, and loop/try spans. It retains the ranking-loop certificate checks from v1 and adds a one- or two-integer product environment handed exactly across two or three linear source-keyed sibling handler regions, plus one exact `if/else` predecessor pair before a common successor. The handler value obligation remains `unknown` after structural convergence; only `analyzeRefinementActionBodiesWithZ3` can upgrade it by independently proving every emitted member expression equivalent to the declared action. Solver failure downgrades the whole product to `unknown`. | The accepted recurrence, linear region, and single conditional topology are still selected by bounded lowering. Predicate loss, a third changed state, a fourth linear region, arbitrary heap aliasing, cross-member recurrences, general recurrence fixed points or widening, arbitrary branch fan-out/region counts, irreducible loops, Program/external-action support for these first CFG artifacts, and unsupported expressions remain outside the claim. |
