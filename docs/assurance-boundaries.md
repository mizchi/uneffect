...
| Generator effects | Direct iterator consumers expose `iteratorEffectParameters`; `effect_parameter iterator extends ...` bounds lazy effects. Symbol-resolved wrappers forward constraints and resolved call sites check generator-body effects, TypeChecker-identified standard strings/collections/typed arrays, or `unknown`. Standard engine-owned iterables instantiate an empty lazy effect set. A finite static property such as `options.values` is represented by `propertyPath` and can be projected through a direct object literal or single-use immutable binding. | Forwarding through arbitrary dynamic dispatch, escaped iterators, computed properties, getters, mutated/reused containers, and an opaque or user-defined iterable argument. A same-named user class is not a standard-iterator exemption. Normal `effect` declarations bound the function body, not its iterator parameter. |
| Hoare contracts | Individual emitted obligations whose artifact status is `verified`. Supported return and loop completion points carry `uneffect-contract-control-flow/v1`, a source-stable block identity, completion kind, and the exact path assumptions asserted for that obligation. The bounded exception extension records path-local caught/escaping synchronous `Throw<E>` and distinct `Reject<E>` edges, executes supported `finally` completion overrides, binds scalar explicit-throw payloads in catch, and binds synchronous escapes to the enclosing Program effect summary; a missing escaping synchronous effect makes the artifact `unknown`. TypeChecker-resolved Promise-returning calls may contribute explicitly `trusted` rejection/throw edges from `temporal_contract`; the solver result is conditional on that declaration rather than proof of the callee implementation. A scalar await nested in the supported expression tree may instantiate a trusted callee postcondition and records it under `relationalCalls`; every callee precondition is a separate call-site implication obligation over the exact caller path. Direct async return of that Promise, or of its supported stored immutable binding, forwards fulfillment and rejection without incorrectly letting a synchronous catch consume the rejection. Same-Program acyclic source edges promote only after all local and transitive obligations verify; failures downgrade callers, while external declarations and cycles remain trusted. The v1 edge binds declaration file/span/SHA-256 and TypeScript version. The separate package-summary v1 envelope validates fully verified direct exports against exact producer package/compiler/source/signature/artifact evidence, but is not yet a consumer authorization. `uneffect-refinement-action-analysis/v2` separately binds source digest, TypeScript version, and loop/try spans. Direct and the one supported handler-backed affine loop share `scalar-recurrence-fixed-point`; the handler form adds source-bound predecessor/payload/snapshot/finally evidence and retains an eight-member compatibility cap. Ordinary direct loops admit at most two integer members plus an ordered `controlJoins` union containing either at most two sequential, distinct, loop-invariant Boolean diamonds, one unchanged-integer `switch` with two non-negative numeric-literal cases, explicit default, and no fallthrough, or exactly one Boolean diamond followed by that switch. One separate three-member family admits a ranking state, one constant-delta driver, and one dependent with a single source-ordered upper-triangular affine edge; `affineDependencies` distinguishes entry from updated-driver reads. Structural convergence remains `unknown`; only `analyzeRefinementActionBodiesWithZ3` can upgrade the value or recurrence after independently checking every member and ranking obligation. Selector/case/predecessor/order and scalar update-order correlation are structural, while Z3 checks the emitted composed recurrence equations. Solver failure downgrades the whole product to `unknown`. | The contract control-flow artifact describes Uneffect's restricted neutral CFG; it is not an export of TypeScript's private compiler CFG. Direct builtin `Promise.reject(value)` and explicitly summarized Promise-returning calls are modeled; unannotated rejection inference, property/destructured awaited targets, opaque catch payloads, consumer linkage from persisted summaries through `.d.ts`, export maps, installed tarballs, and publisher identity, and general exception fixed points remain outside this contract claim. The accepted recurrence, handler topology, and direct loop joins are still selected by bounded lowering. The eight-member handler cap is compatibility capacity, not general product widening. Selector reuse, loss, collision, or mutation, switch-then-if order, fallthrough, signed/dynamic/excess cases, a third direct-loop changed state outside the exact three-member family, multiple/cyclic/nonlinear scalar edges, a third join/diamond, a fourth linear region, arbitrary heap aliasing, self-amplification, general recurrence widening, nested branch fan-out, nested or irreducible loops, Program/external-action support for these first CFG artifacts, and unsupported expressions remain outside the claim. |
...
| Temporal logic | A generated artifact plus the reported backend result and bounds/fairness assumptions. Bounded dynamic Map traces may use multiple direct scalar keys after separate initiation; preservation is either independent per key or one explicitly named jointly-inductive membership conjunction. | An unbounded proof when only simulation or bounded checking ran; compound keys, failed/partial initiation, failed group preservation, and arbitrary collection correlations. |
...
| Native frontend parity | `semanticEquivalent: true` establishes semantic agreement for the emitted neutral-IR fields. An in-process exporter result plus `requireCorsaCheckerFacts` establishes that supported top-level function declarations, single immutable arrow/function-expression bindings, identifier-named methods of top-level classes, named-function overload candidates/selections, type/trivia, direct intra-project calls, reviewed builtin effect atoms (`console.log`, global `fetch`, and exact named async-fs `readFile`/`access`/`readdir` reads plus `writeFile`/`appendFile`/`mkdir` writes), one exact function-local shorthand `const { readFile } = await import("node:fs/promises")`, direct unconditional `await` observations, direct awaits beneath exactly one enclosing `if` branch, direct awaits in exactly one catch-protected try block, and unconditional direct Promise-call returns with at most one Promise assertion came from full Corsa parser services. Single-if evidence includes owner-local file-offset condition identity, polarity, and one control path; caught-await evidence records `catchesRejection: true`; the supported dynamic import records its own await span; owner plus project-wide operation span distinguish cross-file records. Promise metadata comparison covers all observation kinds because normalized projection omits some evidence. | Other filesystem operations/import forms, renamed/namespace/non-literal/mutable/multi-binding dynamic imports, general module initialization/resolution, path scopes, compound read/write calls, computed or polymorphically dispatched methods, callback timing, method/generic overload edge cases, nested/loop handler observations, awaits in catch/finally, conditional/non-call/nested-wrapper return observations, rejection-binding ownership, resource facts, and other neutral-IR domains are not checker-exported. Explicitly annotated computed methods and unsupported Promise shapes make comparison fail coverage/evidence rather than being flattened. Calls or awaits inside unsupported nested functions are omitted rather than presented as immediate outer work. Cloned/persisted facts are unauthenticated, and there is no signed evidence format or whole-program Corsa parity. |
| ESM initialization order | A `uneffect-module-order/v1` artifact with `evidence: "verified"`, for its represented acyclic local graph or synchronous side-effect-import simple ring. The separate experimental `uneffect-module-order/v2` additionally admits one runtime-present source-local immutable Boolean selector with one direct conditional TLA, retaining false/resume completion and terminal rejection through a budgeted CFG postcondition. A `uneffect-workspace-module-order/v1` artifact composes one direct exact child declaration/source with one straight-line TLA into one synchronous importer. Every accepted edge retains source/declaration/compiler evidence and semantic rule. | Runtime-binding, self, branching, multi-edge, or asynchronous cycles; sibling initiation during suspension; conditional/looping TLA beyond the single v2 shape; ambient/imported/mutable/written selectors; await-then-throw; multiple/transitive project dependencies; transformed declarations; decorators/classes; dynamic/external bodies; deployment scheduling; concrete Boolean-path feasibility; or proof that normal completion occurs. |

Update to the compact Hoare row: persisted scalar package contracts now have a
