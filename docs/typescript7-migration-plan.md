...
検査済みrequiresと分岐条件の引数/literal比較による、経路別の区間絞込みも接続した。
条件自体の安全性は、その条件全体を仮定する前に検査する。
本体と契約式の短絡評価では、検査済み左辺の真偽を右辺の値域に反映する。
変数同士の比較も両辺に伝播し、比較の連鎖を最大16走査で反映する。
1変数への定数加減算・符号反転も、元の演算の安全性を検査してから区間へ逆算する。
複数変数の式や乗除算からの絞込みは残る。直接呼出は同一snapshot内の引数なし・単一return・
契約付きcalleeを1段だけ展開する。複数引数・再帰・動的dispatchはunknownのままにする。
