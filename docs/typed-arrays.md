...
suppressed, but the obligation remains visible for audits. This initial form is
function-wide and affects only typed-array obligations. It is not yet a generic
Lean-style `sorry`, has no statement-level scope, and is not authorization for
proof-only optimizer rewrites.
