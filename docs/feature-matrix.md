...
| Promise and resource safety | Partial | Floating rejections, major Promise combinators, selected `try`/`catch`, explicit-throw ownership routing, `using`, `await using`, reverse disposal order, and selected exactly-once obligations have executable models. | Expression-level throw edges and these analyses do not yet share one general control-flow fixed point. See [#9](https://github.com/mizchi/uneffect/issues/9). |
