...
| Native Rust/Corsa frontend | Partial | A neutral IR, Rust parser/lattice, source spans, transitive reference parity, mandatory producer provenance, and a real corsa-bind single-file exporter for top-level named functions/direct local calls/type text/trivia exist. `requireCorsaCheckerFacts` passes only with checker-backed input. | Multi-file identity, methods, arrows, callbacks/timing, overloads, Promise/resource records, and the rest of the neutral IR are not checker-exported. See [#8](https://github.com/mizchi/uneffect/issues/8). |
