...
| Capability effects | Tested fragment | Declared effects are upper bounds; inferred effects propagate through supported symbol-resolved function call graphs; missing and unused effects are diagnosed. Builtin and registered user-defined effects use structured sets. | Top-level module initialization/import evaluation, dynamic dispatch, reflection, proxies, unknown callbacks, and native addons remain uncovered or conservative boundaries. |
