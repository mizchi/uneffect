...
| Cross-project scalar refinement | Tested fragment | A direct project reference can compose one locally verified, unguarded scalar child action into an annotated parent action. The `refinementComposition` ledger retains producer/consumer compiler/config provenance and exact declaration digests; the parent action is revalidated after summary substitution. | Guarded actions, indirect/higher-order calls, collection updates, abstraction transforms, ambiguous exports, adapter/version drift, and non-TypeScript declaration transforms remain non-proofs under [#20](https://github.com/mizchi/uneffect/issues/20). |
