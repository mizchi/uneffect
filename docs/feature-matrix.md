...
| Native Rust/Corsa frontend | Partial | A neutral IR, Rust parser/lattice, source spans, transitive inferred-effect/call/event reference parity, and mandatory producer provenance exist. | Actual facts are not yet exported from the corsa-bind checker bridge. `requireCorsaCheckerFacts` therefore fails closed. See [#8](https://github.com/mizchi/uneffect/issues/8). |
