...
| TypeScript-to-model refinement | Partial | Selected scalar/record/Set/Map updates, conditionals, scalar switch fallthrough and direct return/throw entries, bounded ascending loops, finite numeric/boolean literal `for...of`, acyclic helpers, and a return/throw/catch/finally completion subset are compared with model actions. Finite-loop early returns suppress later iterations without skipping the current iteration's `finally`. Pure return expressions are validated before their result is discarded; tracked `int`/`bool` throw payloads, including normalized numeric/boolean literals, can bind catch-local scalar control flow across direct, conditional `if`, and scalar `switch` throws. Scalar fields of direct or conditionally joined normalized object-literal payloads can also be projected in `catch`. | Dynamic/spread/destructured/`for await` iteration, return-result equivalence, string/null payloads, missing or dynamically shaped object fields, effectful or unresolved completion expressions, labels, general exception-aware CFG fixed points, mutable aliases, and dynamic dispatch remain fail-closed. See [#3](https://github.com/mizchi/uneffect/issues/3). |
...
| Native Rust/Corsa frontend | Partial | A neutral IR, Rust parser/lattice, deterministic project-wide UTF-8 spans, transitive reference parity, mandatory producer provenance, and a real corsa-bind multi-file exporter for top-level function declarations plus single immutable arrow/function-expression bindings, direct intra-project calls, type text, and trivia exist. Unsupported nested callbacks do not leak their calls into an outer immediate-call summary. `requireCorsaCheckerFacts` passes only with authenticated checker-backed input. | Methods, nested callbacks/timing, overloads, Promise/resource records, and the rest of the neutral IR are not checker-exported. See [#8](https://github.com/mizchi/uneffect/issues/8). |
