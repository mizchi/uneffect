...
| Fetch, DOM, and transfer ownership | Tested fragment | Fetch method/URL scopes, reviewed attribute/tree/text methods, compound DOM calls, `NamedNodeMap` origin projection, markup serialization/parsing, layout metrics, reviewed Web IDL property access, and Worker transferable invalidation have regression coverage. | Parent-mutating `outerHTML`, reassigned/escaping live collections, unreviewed Web IDL members, full shared-memory, and Atomics semantics are not modeled. Dynamic DOM keys fall back to `Dom<All, Scope>`. |
