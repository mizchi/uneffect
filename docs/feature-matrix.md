...
| --- | --- | --- | --- |
| Corsa semantic-query API contract | Tested fragment | `@mizchi/uneffect/corsa/api` publishes a versioned immutable descriptor binding the Corsa/compiler/project identities, root files, supported semantic-query capabilities, and limitations. Position/batch symbols and types, bounded aliases, module exports, type properties/symbols, assignability, lifecycle closure, and the bounded builtin classifier have positive and fail-closed tests without constructing a JavaScript TypeScript Program. | Syntax traversal remains out of band, builtin classification is bounded, and broader effect/CFG parity remains [#8](https://github.com/mizchi/uneffect/issues/8). |
