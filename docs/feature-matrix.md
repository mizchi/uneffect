...
| --- | --- | --- | --- |
| Corsa semantic-query API contract | Tested fragment | `@mizchi/uneffect/corsa/api` publishes a versioned immutable descriptor binding the Corsa/compiler/project identities, root files, supported semantic-query capabilities, and limitations. Its Oxc traversal side emits versioned source/parser-bound syntax facts; class and anonymous callback ownership is preserved, while unsupported computed/non-static targets, tagged templates, and dynamic imports block Corsa checking. | Syntax traversal remains out of process from Corsa, builtin classification is bounded, and broader effect/CFG parity remains [#8](https://github.com/mizchi/uneffect/issues/8). |
