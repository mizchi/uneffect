...
| React function components | Partial | Explicitly annotated TSX components separate replayable render, inline JSX events, layout/passive Effect setup, and cleanup. The tested fragment rejects direct render capabilities, selected non-idempotent reads, identifier-prop mutation, conditional built-in Hooks, and unmatched capability acquisition. | Custom/re-exported Hooks, dependency and stale-closure analysis, resource identity, state/context/ref flow, Suspense/concurrent lifecycle models, and server components remain open. See [#16](https://github.com/mizchi/uneffect/issues/16). |
