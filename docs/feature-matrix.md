...
| Capability effects | Tested fragment | Declared effects are upper bounds; inferred effects propagate through supported symbol-resolved function call graphs. Source-attributed `<module>` summaries cover direct operations, resolved calls/overloads, known inline callbacks, immutable local/imported callback identifiers, and static local import cycles as may-effect sets. Reassigned callback live bindings fail closed. Missing and unused function/`module_effect` bounds are diagnosed. Builtin and registered user-defined effects use structured sets. | Exact ESM/top-level-await temporal order, external-package initialization, conditional dynamic imports, mutable or dynamically selected callbacks, dynamic dispatch, reflection, proxies, and native addons remain unknown or conservative boundaries. |
...
| Evidence and diagnostics | Partial | Artifacts bind inputs, configuration, tool versions, and outcomes. Diagnostics include source locations and replayable witnesses for supported checks. `check --assurance no-unknown` rejects unknown effect/contract evidence; `declared` additionally requires declaration-checked effect summaries. | Assurance profiles cover emitted evidence for explicit files and opted-in domains, not arbitrary TypeScript. Z3/Quint artifacts are reproducible evidence, not independently checkable proof certificates. See [#7](https://github.com/mizchi/uneffect/issues/7). |
