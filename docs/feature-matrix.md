...
| Promise and resource safety | Partial | Floating rejections, major Promise combinators, selected `try`/`catch`, explicit-throw ownership routing, structured and statically selected `&&`, `||`, ternary, and nullish-coalescing `never` + `Throw<E>` expression edges, `using`, `await using`, reverse disposal order, and selected exactly-once obligations have executable models. | General expression-level throw edges, nullable-union selection, and these analyses do not yet share one control-flow fixed point. See [#9](https://github.com/mizchi/uneffect/issues/9). |
...
| React function components | Partial | Explicitly annotated TSX components and custom Hooks separate replayable render, inline JSX events, layout/passive Effect setup, and cleanup. Source-local and TypeScript-resolved named-import Hook calls compose. The tested fragment rejects direct render capabilities, selected non-idempotent reads, identifier-argument mutation, conditional/unresolved/directly-recursive Hooks, unmatched capability acquisition, wrong/duplicate local resource cleanup, missing inline Hook dependencies, and exposes production/Strict-Mode replay projections. | Barrel/namespace/default Hook resolution, indirect recursion, symbol-resolved dependency aliases and custom stability, general resource alias/ownership flow, state/context/ref flow, Suspense/concurrent lifecycle models, and server components remain open. See [#16](https://github.com/mizchi/uneffect/issues/16). |
