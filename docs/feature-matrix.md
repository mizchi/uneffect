...
| Fetch, DOM, and transfer ownership | Tested fragment | Fetch method/URL scopes, receiver-scoped reviewed attribute/tree/text methods, reviewed attribute collection, tree-topology, `textContent`/`nodeValue`/`CharacterData.data` text access, `HTMLInputElement.value` property access, and Worker transferable invalidation have regression coverage. | Unreviewed Web IDL members, live-collection mutation through returned identities, full shared-memory, and Atomics semantics are not modeled. Dynamic DOM keys fall back to `Dom<All, Scope>`. |
