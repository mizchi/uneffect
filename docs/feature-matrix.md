...
| Cross-project scalar refinement | Tested fragment | A direct project reference can compose a locally verified scalar child action into an annotated parent action. Unguarded calls may be direct or pass through one TypeChecker-resolved, write-screened source-local function helper; `callPath` records the bounded edge. A guarded action composes only through a sole direct wrapper call and retains its formatted guard in `refinementComposition`. The ledger also retains producer/consumer compiler/config provenance and exact declaration digests; the parent action is revalidated after summary substitution. | Guarded indirect calls, higher-order/reassigned/cyclic or two-level helpers, guarded wrappers with additional work, collection updates, abstraction transforms, ambiguous exports, adapter/version drift, and non-TypeScript declaration transforms remain non-proofs under [#20](https://github.com/mizchi/uneffect/issues/20). |
