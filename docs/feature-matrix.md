...
| Temporal models and Quint | Partial | Typed state/actions, safety, bounded reachability, selected liveness fragments, replay, and reviewable Quint generation are implemented for documented finite/bounded models. Finite `Map.getOrElse(key, fallback)` is total across Quint, runtime assertions/replay, and bounded Z3 for literal keys and one independently proved immutable finite-Set membership fragment; evidence names the dynamic key domain. Record fallbacks are supported. | Ordinary `Map.get` remains partial and requires a matching conjunctive domain guard. Dynamic construction/mutation, compound or multiple Map keys, mutable/ambiguous domains, failed induction, and solver failure remain `unknown`; general formulas, synthesis, collection correlations, and broader TLC interoperability remain open. See [#2](https://github.com/mizchi/uneffect/issues/2), [#5](https://github.com/mizchi/uneffect/issues/5), and [#43](https://github.com/mizchi/uneffect/issues/43). |
