...
| Native Rust/Corsa frontend | Partial | A neutral IR, Rust parser/lattice, deterministic project-wide UTF-8 spans, transitive reference parity, mandatory producer provenance, and a real corsa-bind multi-file exporter for top-level named functions/direct intra-project calls/type text/trivia exist. `requireCorsaCheckerFacts` passes only with authenticated checker-backed input. | Methods, arrows, callbacks/timing, overloads, Promise/resource records, and the rest of the neutral IR are not checker-exported. See [#8](https://github.com/mizchi/uneffect/issues/8). |
