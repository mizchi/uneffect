...
| Fetch, DOM, and transfer ownership | Tested fragment | Fetch method/URL scopes, receiver-scoped DOM call classes (`AttributeRead`/`AttributeWrite`, `NodeRead`/`NodeWrite`, layout, creation, listener, dispatch, and parsing), and Worker transferable invalidation have regression coverage. `Text*` and `Property*` are reserved operation kinds. | Ordinary DOM property getter/setter inference, full shared-memory, and Atomics semantics are not modeled. |
