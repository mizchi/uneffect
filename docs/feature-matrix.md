...
| Contract-derived testing | Partial | Deterministic Vitest generation and shrinking cover scalar machine domains, literal unions, bounded arrays, selected records, optional fields, bounded Set/Map values, supported refinements, and one explicit finite specialization for an exported source-local unary user predicate. The generated test rechecks the real predicate and rejects zero valid candidates. | Cross-file, recursive, higher-order, multi-argument, inferred, and nested predicate specializations and general solver-backed shrinking remain open. See [#4](https://github.com/mizchi/uneffect/issues/4). |
