...
| Cross-project scalar refinement | Tested fragment | A direct project reference can compose a locally verified scalar child action into an annotated parent action. Calls may be direct or pass through one TypeChecker-resolved, write-screened source-local function helper whose body is exactly that call; `callPath` records the bounded edge. A guarded action retains its formatted guard through either supported shape. The ledger also retains producer/consumer compiler/config provenance and exact declaration digests; the parent action is revalidated after summary substitution. | Higher-order/reassigned/cyclic or two-level helpers, helper-local guards or extra/conditional work, guarded wrappers with additional work, collection updates, abstraction transforms, ambiguous exports, adapter/version drift, and non-TypeScript declaration transforms remain non-proofs under [#20](https://github.com/mizchi/uneffect/issues/20). |
