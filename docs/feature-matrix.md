...
| Fetch, DOM, and transfer ownership | Tested fragment | Fetch method/URL scopes, receiver-scoped reviewed attribute/tree/text methods, compound clone/normalize/adjacent-content operations, direct and immutable-alias `NamedNodeMap` origin projection, reviewed property access, and Worker transferable invalidation have regression coverage. | Reassigned/escaping live collections, unreviewed Web IDL members, full shared-memory, and Atomics semantics are not modeled. Dynamic DOM keys fall back to `Dom<All, Scope>`. |
