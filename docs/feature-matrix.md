...
| Capability effects | Tested fragment | Declared effects are upper bounds; inferred effects propagate through supported symbol-resolved function call graphs. Source-attributed `<module>` summaries cover direct operations, resolved calls/overloads, known inline callbacks, and static local import cycles as may-effect sets. Missing and unused function/`module_effect` bounds are diagnosed. Builtin and registered user-defined effects use structured sets. | Exact ESM/top-level-await temporal order, external-package initialization, conditional dynamic imports, dynamic dispatch, reflection, proxies, unknown callbacks, and native addons remain unknown or conservative boundaries. |
