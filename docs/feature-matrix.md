...
| TypeScript-to-model refinement | Partial | Selected scalar/record/Set/Map updates, conditionals, scalar switch fallthrough, bounded literal loops, acyclic helpers, and a return/throw/catch/finally completion subset are compared with model actions. | Value returns, effectful throws, labels, general exception-aware CFG fixed points, mutable aliases, and dynamic dispatch remain fail-closed. See [#3](https://github.com/mizchi/uneffect/issues/3). |
