...
This is the release-boundary view of Uneffect 0.3.0 as of 2026-09-05. It condenses
...
| --- | --- | --- | --- |
| Corsa semantic-query API contract | Tested fragment | `@mizchi/uneffect/corsa/api` publishes a versioned immutable descriptor binding the Corsa/compiler/project identities, root files, supported semantic-query capabilities, and limitations. Its Oxc traversal side emits versioned source/parser-bound syntax facts; class, constructor, immediately-invoked, and anonymous callback ownership is preserved. Computed members on checker-resolved non-DOM receivers are admitted (computed calls stay unknown callees), while dynamic keys on DOM or `any` receivers, tagged templates, and dynamic imports block Corsa checking. The default check also reports `bounds/unchecked-index` for unguarded immediate dereferences of `Array`/`ReadonlyArray` elements. | Syntax traversal remains out of process from Corsa, builtin classification is bounded, and broader effect/CFG parity remains [#8](https://github.com/mizchi/uneffect/issues/8). |
| Temporal model facade contract | Tested fragment | `generateTemporalModel` returns `uneffect-temporal-model/v1`; every public temporal, Promise, synchronization, and resource domain is classified as `modeled`, `not-applicable`, or `excluded`. `parseTemporalModelResult` and the published JSON Schema reject unknown fields, invalid enum values, malformed projections, and incomplete coverage inventories. | This stabilizes the facade and evidence boundary, not generated Quint text or general host semantics. Broader temporal, collection, and event-loop reasoning remains [#2](https://github.com/mizchi/uneffect/issues/2), [#5](https://github.com/mizchi/uneffect/issues/5), and [#10](https://github.com/mizchi/uneffect/issues/10). |
...
| Fetch, DOM, browser storage, same-realm globals, external scripts, and transfer ownership | Tested fragment | Fetch method/URL scopes, reviewed attribute/tree/text methods, compound DOM calls, `NamedNodeMap` origin projection, receiver- and parent-scoped markup operations, layout metrics, reviewed Web IDL property access, and Worker transferable invalidation have regression coverage. `Document.cookie` and Web Storage symbol identities produce separate read/write permissions. Literal cookie assignments and literal/finite-union Web Storage keys retain finite authority sets; aggregate reads, indexed enumeration, and clear remain broad, while dynamic keys fail closed. `GlobalVarsRead<KeySet>` / `GlobalVarsWrite<KeySet>` preserve literal and finite-union keys for TypeChecker-resolved current-realm globals and immutable local aliases; dynamic keys fail closed. TypeChecker-resolved `Navigator.sendBeacon` and global `new WebSocket(...)` emit `Net` plus separate exact/unknown transport evidence. One direct, same-block `createElement("script")` configuration followed by `appendChild` emits URL-, SRI-, CORS-, and host-scoped `ScriptLoad`, `ExecuteExternalCode`, and `Net` effects. Effect summaries retain direct and propagated transport evidence separately from the common `Net` set. | Cookie path/domain and storage origin/area identity are not scoped yet. Global property effects do not equate module variables, iframe realms, declaration merging, or arbitrary same-spelled objects; nested heap mutation remains a separate `Mutate` effect. WebSocket event/message/reconnect lifecycle is not modeled by connection evidence. The script fragment proves the loader boundary, not downloaded code. Aliases, escape, conditional configuration, event timing, redirects, CSP, child scripts, parent-presence refinement, reassigned/escaping live collections, unreviewed Web IDL members, full shared-memory, and Atomics semantics are not modeled. Dynamic DOM keys fall back to `Dom<All, Scope>`. |
...
| Hoare contracts and Z3 | Partial | Restricted integer/machine-number preconditions, postconditions, invariants, assignments, selected path-sensitive control flow, property model enumeration, and typed-array obligations generate reviewable SMT-LIB. Return/loop obligations retain versioned source-block and path-condition evidence. Recursive scalar ternaries in returns, initialized bindings, and plain assignments split into explicit true/false path assumptions. One-to-eight-clause numeric/Boolean literal `switch` supports explicit case/default entry, fallthrough, target-owned unlabeled `break`, and throw-to-catch/discharge composition. A TypeChecker-validated finite readonly string discriminant, including immutable aliases, reuses its exactly-one Boolean family for switch entry/default/fallthrough and narrowed payloads without inventing an SMT string. Invariant-backed `while`, canonical scalar `for`, and `do...while` share target-owned unlabeled break/continue; continue proves the back edge, for applies supported scalar updates before the back edge, break exits, and do-while exits only after one body. Scalars without syntactic writes retain their entry expression and path facts across those loops. Stable TypeChecker-resolved Program-visible callables, including `const` aliases and named imports, compose symbol-derived captured-write sets through an acyclic call graph and reviewed Math leaves when every concrete `.ts` body is present and error-free; recursion, dynamic callback dispatch, member access/construction, deletion, `.d.ts`-only/opaque bodies, and unresolved calls conservatively invalidate the entire frame. Nested function declarations themselves are non-executing CFG statements. A `for` initializer accepts existing-binding scalar assignments or one or more initialized identifier declarations evaluated left-to-right; `let`/`const` shadows are restored together and `var` remains function-scoped. Identifier-only statement/header `++`, `--`, `=`, `+=`, `-=`, and `*=` use one symbolic updater; the reviewed signed-Int `%=` fragment and comma-separated header updates share its ordered path handling. Exact error-free TypeScript Programs additionally contribute finite safe-integer literal-union parameter facts through local or imported type aliases, direct nullish guards over number/null/undefined unions, direct `typeof` number guards over number/string unions, and direct equality guards over a two-to-eight-member readonly string-discriminated object union. Discriminants lower to an explicit exactly-one Boolean family rather than encoding the object as a scalar. The object may flow through an immutable identifier-only `const` alias chain resolved by TypeChecker symbol identity; one unambiguous one-to-four-segment readonly parameter property path may expose that root when first selected into the same alias form. After such a guard, a readonly dot-property path whose terminal flow type is a safe-integer/Boolean literal lowers to a source-span constant; a terminal scalar `number`/`boolean`/`Int`/`Nat`/`Float` lowers to an independent member-scoped solver variable with its domain constraints. Every intermediate property must be readonly. `const` object destructuring of those narrowed scalar payloads is supported for direct or readonly nested sources and renamed bindings, with exact binding/property identity. Narrowed readonly tuple payloads additionally support fixed non-negative literal index reads and flat `const` array destructuring. TypeChecker-resolved named, namespace, default, or import-equals `node:assert/strict` assertion calls, plus named `ok` and the default callable from `node:assert`, split into a normal condition path and a trusted `Throw<AssertionError>` path, which catch may discharge; the builtin boundary is retained in the assumption ledger. Coercive assertion helpers are not narrowing facts. A bounded exception CFG routes direct throws and direct TypeChecker-resolved `never` + declared `Throw<E>` calls through catch, records path-local discharge and escaping edges, and joins them with the enclosing effect summary. Supported `finally` blocks preserve or override normal/return/throw/rejection completion, scalar explicit-throw payloads bind catch identifiers, and directly awaited TypeChecker-identified builtin `Promise.reject(value)` creates a distinct `Reject<E>` edge. The exact standard `Promise.resolve(value)` supplies a verified scalar `result === value` fulfillment relation when the argument itself is numeric or Boolean; Promise/thenable assimilation remains fail-closed. TypeChecker-resolved Promise-returning calls may use trusted `temporal_contract rejects E` and `temporal_contract throws E` declarations to branch into fulfillment, rejection, and synchronous-throw paths. One direct scalar `const value = await call()` or `return await call()` may instantiate a trusted callee `contract ensures` relation; the evidence records both spans and exact clauses. A source-program acyclic fixed point promotes same-file or cross-file edges only after all callee and transitive dependency obligations verify, while failures downgrade callers and cycles remain trusted; v1 call evidence binds declaration file/span/SHA-256 and TypeScript version. A standalone `uneffect-contract-summary/v1` producer/validator binds fully verified direct exports to package/version, compiler options, source/declaration/signature digests, clauses, and artifact IDs. Missing escaping synchronous effects downgrade the contract artifact to `unknown`; Promise rejection stays separate. Every current solver client prefers native Z3 in `auto` mode and falls back to bundled WASM only for infrastructure failure. | General TypeScript narrowing, mutable/destructured object aliases, computed/cyclic/over-depth property roots, nested discriminants, composite object/array payload values, user-defined assertion functions, arbitrary/call-conditioned/effectful conditional expressions, heap aliasing, termination, general loop fixed points, dynamic/open/mutable/unknown string switch cases, mixed-sort switch cases, labels, recursive callable write-set fixed points, opaque dependency bodies, destructured/uninitialized declaration headers, and complex/effectful loop expressions are not proved. Property/logical mutation, arbitrary comma values, `/=`, and `%=` outside the reviewed signed-Int/literal-divisor fragment remain unsupported; general JS number division/remainder is not silently equated with Z3 integer arithmetic. Pre-narrow, mutable, defaulted, rest, or computed object destructuring is not admitted; ordinary/mutable arrays, dynamic tuple indexes, holes, defaults, rest, and nested tuple bindings are also outside the subset. Contract exception lowering does not yet infer general unannotated Promise rejection/fulfillment or admit property/destructured awaited targets, opaque catch payloads, consumer linkage from persisted summaries through `.d.ts`, export maps, installed tarballs, and publisher identity, or general exception fixed points. Temporal and relational summaries are trusted declarations rather than implementation proofs. Plain `number` receives an unconstrained solver value rather than a range; unions larger than sixteen, non-integer literals, a source mismatch, or a Program with TypeScript errors contribute no narrowing fact. See [#25](https://github.com/mizchi/uneffect/issues/25) and [#24](https://github.com/mizchi/uneffect/issues/24). |
| TypeScript CFG bridge | Experimental tested artifact | Contract runtime exit checks combine Uneffect's structured CFG with TypeScript's public `noImplicitReturns` semantic diagnostics. Versioned evidence binds aggregate/per-source digests, compiler version/options, source-qualified function identity, endpoint coverage/exclusions, neutral endpoint, and parity status. A strict parser and published schema reject forged counts, source identities, parity, spans, and missing unknown reasons. Existing Programs are reused across files by declaration-node identity. | TypeScript's internal `flowNode` is observation-only because it is not a public Compiler API. Mutable bindings, incompatible compiler options, setter postconditions, dynamic computed names, arbitrary block/edge exposure, and general value/path proofs remain unknown or rejected. See [TypeScript control-flow bridge](./typescript-control-flow.md). |
| Temporal models and Quint | Partial | Typed state/actions, safety, bounded reachability, selected liveness fragments, replay, and reviewable Quint generation are implemented for documented finite/bounded models. `generateTemporalModel` composes user temporal state and callback summaries with supported Promise, timer, microtask, and Web/Node event-loop observations through one host-aware public entry. A selected root's binding-level Promise ownership lowers through the common resource IR: observed/transferred bindings satisfy `promiseOwnershipSafe`, while a floating binding is a counterexample. A directly bound builtin `new Promise` and supported immutable aliases normalize to one TypeChecker declaration identity linked to the host settlement transition; the exact link is exposed in `synchronizations`. Supported immutable AbortController/fetch bindings add the cancellation/external-settlement/Promise/body product to the same result. A selected root's `using`/`await using` acquisition and disposal lifecycle is co-verified through the same result and project pipeline. Straight-line and bounded non-loop conditional `await using` have a resource/host product; initializer failure skips later acquisitions and cleans up the acquired prefix, conditional source paths explicitly acquire-or-skip and release-or-skip, and async disposal resumes inside a microtask checkpoint. One contiguous source-loop acquisition group has explicit zero/repeat/exit generation transitions. Disposal throw/reject paths retain a finite failure count and multiple-failure suppression invariant. Scalars include `int`, `bool`, and exact string identities; strings support equality and finite Set/string-keyed Map membership without numeric coercion. Finite `Map.getOrElse(key, fallback)` is total across Quint, runtime assertions/replay, and bounded Z3 for literal keys and proved immutable finite-Set membership fragments. Multiple direct scalar keys first use independent proofs; one bounded failover family may instead prove the complete membership conjunction, with joint assumptions named in evidence. Record fallbacks are supported. An exact finite Set of directly written scalar-field record literals can be decoded from native/WASM Z3 and normalized with Quint/TLC replay. | Promise ownership is binding-level; external producers and unsupported dynamic/escaping aliases retain `promise-host-synchronization`, and reaction-job ordering is not generally proved. External/dynamic fetch signals retain `abortable-fetch-synchronization`. Multiple, nested, non-contiguous, or non-stack repeated resource regions remain outside the resource/host product as `resource-host-scheduling`; arbitrary callback interleavings remain `resource-host-callback-interleavings`. Direct awaited initializer fulfillment/rejection uses the microtask lane, but indirect wrapper/thenable timing and concrete runtime `SuppressedError` payload value/object identity are not represented; the finite origin/parent chain is checked. Direct async/Promise/event-loop Quint generators are experimental under `@mizchi/uneffect/experimental`. String arithmetic/order, control characters in SMT strings, unbounded string generation, and an incomplete string observation universe are unsupported. Record-valued Set extraction does not admit state-derived/dynamic elements, record spreads, nested collection fields, or an unbounded universe. Ordinary `Map.get` remains partial and requires a matching conjunctive domain guard. Dynamic construction/mutation, compound keys, a key with a missing/mutable/ambiguous domain, failed initiation or joint preservation, and solver failure keep the whole observation universe `unknown`; arbitrary collection correlations, formulas, synthesis, and broader TLC interoperability remain open. See [#63](https://github.com/mizchi/uneffect/issues/63), [#2](https://github.com/mizchi/uneffect/issues/2), [#5](https://github.com/mizchi/uneffect/issues/5), and [#59](https://github.com/mizchi/uneffect/issues/59). |
...
the same identity rule, as does `new root.NestedConstructor()`. Calls remain `trusted`, not `verified`,
...
source identity. `generateHostTransitionModel` also projects the reaction into
the executable Web/Node Quint Promise-chain state machine with explicit
...
Standard `Promise.resolve` and `Promise.reject` producer recognition follows
the TypeChecker-resolved lib declaration through immutable callable aliases;
it does not depend on the local identifier spelling.
A local `async` function declaration, `const` arrow, or `const` function
expression with identifier-only parameters and one pure scalar return
expression closed over those parameters supplies a verified fulfillment
...
wider unions remain `unknown`.
Exact numeric or Boolean `T | undefined` additionally accepts a `typeof`
equality/inequality comparison with `"undefined"` as the inverse/direct
presence test. A union that also
contains null remains `unknown`, because the current presence fact deliberately
does not pretend to distinguish null from undefined.
Nullable Boolean facts relate presence to truthiness with `!defined =>
...
Direct Int `%` and statement/header `%=` share JavaScript's signed-remainder
...
tracked scalar; each incoming path is scoped independently, and normal and
abrupt exits restore that path's outer value. The same rule applies to a catch
...
updates the existing binding, while `var` remains visible after the loop. An
...
that integer/real contract abstraction. Immutable direct property,
destructuring, and identifier-only callable aliases retain the reviewed Math
identity; mutable, computed, shadowed, and dynamic aliases remain `unknown`.
Standard-library `Math.floor`, `ceil`, `trunc`, and `round` also lower through
SMT integer conversion and explicit sign paths. Their claim is the finite Real
abstraction, not JavaScript NaN, infinity, or negative-zero equivalence.
Standard `Math.sign`, including immutable callable aliases, splits the same
abstraction into negative, zero, and positive result paths.
Bounded numeric `**` and standard `Math.pow` expand a literal exponent from zero
through eight into repeated multiplication after evaluating the base once.
Dynamic, negative, fractional, larger, shadowed, or effectful forms remain
`unknown`.
Ordinary identifier `+=`, `-=`, and `*=` statements pass their right operand
through this same path-sensitive scalar evaluator and require matching numeric
IR sorts. Ternaries, reviewed Math calls, and the supported remainder fragment
therefore retain their path assumptions without duplicating their semantics;
loop-header arithmetic updates otherwise remain deliberately single-path.
Direct JavaScript `/`, general `%`, `/=`, and `%=` outside the reviewed literal
fragment are deliberately `unknown`;
they are not mapped to SMT real division or Euclidean `mod`. One exception is
an Int-valued dividend modulo a direct nonzero safe-integer literal: the CFG
