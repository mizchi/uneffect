...
| **Total additive effort** | 12 open Issues | **49–98 engineer-weeks** | Low |
...
- **All currently requested work: 49–98 engineer-weeks.** This includes
...
| B — reusable analyzer core | #23; first bounded child slices of #24 and #8 | General dynamic dispatch, complete Corsa parity, specialized products | 7–16 additional weeks | Confirm that new domains use shared CFG/alias/frontend facts rather than shape-specific walkers. |
...
49–98 week total remains additive and intentionally ignores speculative
parallel speed-up.
...
P2.3 then applies the same named worklist budget to the telemetry routing
`switch`/`catch`/mandatory-`finally` join. It retains normal, return, and throw
reachability, consumes only caught throw edges, and exposes every block state in
the strict artifact. The current classifier is deliberately direct-shape only;
it is not yet general AST-to-basic-block lowering.
...
| ---: | --- | --- | ---: | --- | --- |
| 1 | #23 general refinement CFG | L | 1–4 weeks total remaining | Reusable AST-to-basic-block lowering for one nested application handler family | Nested/arbitrary joins, recurrence widening, and irreducible control |
...
2. **General analysis foundation (7–16 additional weeks):** continue with #23,
...
| Current slice | P2.4 reusable nested-handler lowering | 1–2 engineer-weeks | Replace the direct classifier with AST-to-basic-block construction for one nested application family and retain unsupported irreducibility as `unknown`. |
...
The #23 issue-level estimate is now 1–4 weeks after completing P2.3. The direct
handler seed proves that completion sets fit the shared worklist and named
budget, but its dedicated switch classifier does not reduce the uncertainty
around nested/arbitrary lowering or general recurrence widening.

## Backlog interpretation

- **Next implementable result:** P2.4 in #23, estimated at 1–2 engineer-weeks.
- **Next foundation checkpoint:** finish and re-estimate #23, then activate a
  ...
  foundation is 7–16 engineer-weeks in total, including #23.
...
- **Entire open backlog:** 49–98 engineer-weeks. This is an additive research
