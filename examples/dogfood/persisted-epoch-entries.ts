/* uneffect:temporal state epochs: Map<int, int> */ /* uneffect:temporal init epochs = Map([[1, 0]]) */ /* uneffect:temporal action addFallback: epochs' = epochs.put(2, 1) */ /* uneffect:temporal action removePrimary: epochs' = epochs.remove(1) */ /* uneffect:temporal action clearEpochs: epochs' = Map([]) */ /* uneffect:temporal action upsertPrimary: epochs' = epochs.put(1, 5) */ /* uneffect:temporal invariant primaryPresent: epochs.keys().contains(1) */ /* uneffect:temporal invariant hasEpochs: epochs.size() > 0 */ /* uneffect:temporal invariant nonNegativeEpochs: epochs.values().forall(epoch => epoch >= 0) */ /* uneffect:refinement abstraction persistedEpochs@1 epochs = Map(storage.epochEntries) */
