...
dogfood-leaf:
    pnpm tsx src/cli.ts check --assurance no-unknown src/static-evaluation.ts src/project-coordinates.ts src/disposal-symbols.ts
    pnpm vitest run test/dogfood.test.ts -t "explicit pure boundary|pure construction|disposal traversal"
