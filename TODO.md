...
- [ ] Extend module initialization from may-effect closure to exact ESM evaluation/TLA temporal ordering, side-effectful external packages, conditional dynamic imports, dynamically selected or mutable callback identifiers, and complete class/namespace initialization semantics. Immutable local/imported function identifiers for known callback owners are resolved by TypeChecker identity; reassigned live bindings fail closed.
