...
- [ ] Extend frontend parity from declarations to inferred effects, call edges, ordered events, and real Corsa checker facts. ([#8](https://github.com/mizchi/uneffect/issues/8))
  ...
  - [x] Route the main analyzer's global `Fetch`/`Console` catalog resolution through an optional compiler-neutral semantic sidecar, retaining TypeScript AST traversal, batching symbol positions once per source file, and turning dual-frontend mismatch into an assurance blocker before Corsa becomes authoritative.
