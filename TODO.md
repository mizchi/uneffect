  ...
  - [ ] Detect actions unreachable from the transition system's initial states.
    ...
    - [ ] Synthesize general affine/polyhedral, quantified, and richer field-correlated collection invariants.
      ...
      - [ ] Prove a refinement mapping between adjacent TypeScript implementation updates and temporal model actions; current dogfood checks the model independently.
        ...
        - [ ] Extend invariant-body refinement beyond normalized scalar predicates, immutable local constants, and acyclic TypeChecker-resolved pure helper graphs to collections, direct namespace-property calls, higher-order values, and dynamic dispatch. Logical equivalence within the normalized scalar fragment is now solver-proven opt-in.
          - [ ] Preserve object-parameter substitutions across multiple imported helper layers; scalar argument chains work, but the current normalizer rejects this richer composition conservatively.
