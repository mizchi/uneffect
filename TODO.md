...
2. [ ] [#24](https://github.com/mizchi/uneffect/issues/24) Establish a common region/alias IR for non-escaping immutable aliases,
   ...
   - [ ] [#24](https://github.com/mizchi/uneffect/issues/24) Migrate contract/refinement mutation snapshots.
     ...
     - [x] Admit per-predecessor lexical scalar shadowing while restoring each
     ...
     - [x] Sequence comma-separated scalar assignments and increments in `for`
       expression-initializer/update positions from left to right.
...
- [x] Restore the classified-unknown self-dogfood gate with reviewed, TypeChecker-identified synchronous callback contracts for the TypeScript compiler traversal APIs used internally; same-spelled user APIs remain unknown, and remaining callback/generator unknowns require stable reason codes. ([#21](https://github.com/mizchi/uneffect/issues/21))
...
- [x] Generalize source-local callback Effect parameters to one-level readonly function-valued option properties, including wrapper forwarding and literal/single-use-const projection; keep mutable unresolved properties unknown.
  - [x] Preserve multiple `(index,path)` callbacks on one options argument and converge mutually recursive local forwarding with a bounded finite-summary fixed point.
...
- [ ] Extend frontend parity from declarations to inferred effects, call edges, ordered events, and real Corsa checker facts. ([#8](https://github.com/mizchi/uneffect/issues/8))
  ...
  - [x] Resolve Uneffect's fixed `@typescript/native-preview` prebuilt by default, make the JavaScript TypeScript peer optional for `/corsa/api`-only installs, and prove the first checker-authenticated `Fetch`/`Console` slice rejects same-spelled locals. Keep re-exported `node:fs/promises` aliases fail-closed until Corsa exposes alias canonicalization.
  - [x] Pin the Corsa worker to TypeScript 7 native `lib/tsc` platform packages instead of `@typescript/native-preview`, and classify const-aliased `fetch` through checker type identity so the sidecar no longer needs the TypeScript 6 adapter for that slice. ([#8](https://github.com/mizchi/uneffect/issues/8))
  - [x] Route the main analyzer's global `Fetch`/`Console` catalog resolution through an optional compiler-neutral semantic sidecar, retaining TypeScript AST traversal, batching symbol positions once per source file, and turning dual-frontend mismatch into an assurance blocker before Corsa becomes authoritative. `overlayCorsaBuiltinCatalog` feeds batched `classifyBuiltinCalls` into `analyzeProgramEffects` / `buildProgramCallGraph` when a sidecar is attached; the TypeScript adapter stays as fallback and parity oracle.
  ...
  - [x] Consume `@corsa-bind/napi@1.13.1` named wrappers for `getSymbolsAtPositions`, `getAliasedSymbol`, `getImmediateAliasedSymbol`, `getExportsOfModule`, `getTypesAtPositions`, `getPropertyOfType`, and `isTypeAssignableTo` instead of `callJson` strings, and compare local module export names against the TypeScript 6 checker. ([#8](https://github.com/mizchi/uneffect/issues/8))
  - [ ] [#8](https://github.com/mizchi/uneffect/issues/8) Wrap remaining project-scoped Corsa resolved-signature endpoints with typed adapters before migrating named or re-exported Node builtin effects. Do not add a thin `getSignaturesOfType` wrapper. Direct and two-hop `node:fs/promises` aliases still end at the unknown symbol.
...
- [x] Add multi-file call graphs, aliases, re-exports, methods, arrows, overloads, and callbacks.
  ...
  - [x] Recognize TypeChecker-identified standard iterator result objects across finite conditional/nullish unions, rejecting same-named user iterator classes.
  ...
  - [x] Preserve iterator property paths through verified child-project summaries and exact declaration-output authentication, with parent-side literal specialization.
...
- [x] Extend floating-Promise analysis from expression statements to initialized/deferred local binding ownership, aliases, reassignment loss, and path-sensitive observation.
  ...
  - [ ] Replace the restricted path walker with a general CFG fixed point covering arbitrary `switch`/`try`/`finally` joins and irreducible loops. ([#25](https://github.com/mizchi/uneffect/issues/25))
    ...
    - [x] Lower Promise-ownership `switch` entry, clause fallthrough, and exit
    ...
    - [x] Evaluate guaranteed-throw expression prefixes in source order for
      Promise-ownership if conditions, loop tests, and switch discriminants;
      exclude awaits after the throw from ownership evidence, preserve awaits
      before it, and join dynamic all-throw ternary branches conservatively.
    ...
    - [x] Evaluate Promise-ownership call targets before arguments and arguments
...
- [x] Reconcile same-file relational calls with local callee artifacts to a
  ...
  - [x] Extend that fixed point to synchronous scalar function contracts,
    ...
    - [x] Reuse the closed local scalar-producer inference for unannotated
      ...
      - [x] Synthesize finite guarded inferred summaries for reviewed
        `Math.abs`, one-to-four argument `min`/`max`, `trunc`, and `sign`,
        including their use inside surrounding scalar arithmetic.
