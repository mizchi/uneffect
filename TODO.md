...
- [x] Load consumer compiler options and root-file selection through `check --project <tsconfig.json>` instead of silently applying Uneffect defaults; reject malformed and empty projects.
  ...
  - [ ] Carry the same referenced-config graph into `verifyUneffectProject`, compose cross-project effect/refinement evidence without flattening compiler domains, and validate emitted declaration/build-artifact semantics. ([#20](https://github.com/mizchi/uneffect/issues/20))
