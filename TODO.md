...
- [ ] Extend frontend parity from declarations to inferred effects, call edges, ordered events, and real Corsa checker facts. ([#8](https://github.com/mizchi/uneffect/issues/8))
  ...
  - [x] Consume `@corsa-bind/napi@1.13.0` named wrappers for `getSymbolsAtPositions`, `getAliasedSymbol`, `getImmediateAliasedSymbol`, and `getExportsOfModule` instead of `callJson` strings, and compare local module export names against the TypeScript 6 checker. ([#8](https://github.com/mizchi/uneffect/issues/8))
  - [ ] [#8](https://github.com/mizchi/uneffect/issues/8) Wrap remaining project-scoped Corsa resolved-signature endpoints with typed adapters before migrating named or re-exported Node builtin effects. Latest `@corsa-bind/napi@1.13.0` already has named `getSymbolsAtPositions` / `getAliasedSymbol` / `getImmediateAliasedSymbol` / `getExportsOfModule` on the binary; Uneffect consumes those. Typed adapters for `getTypesAtPositions`, `getPropertyOfType`, and `isTypeAssignableTo` prefer named N-API when present and still fall back to `callJson` on 1.13.0. Do not add a thin `getSignaturesOfType` wrapper. Direct and two-hop `node:fs/promises` aliases still end at the unknown symbol. Published 1.13.0 `dist/index.d.mts` still omits the 1.13.0 method names that `index.d.ts` already declares.
