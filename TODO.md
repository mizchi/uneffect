...
- [ ] Detect solver-level tautologies, inconsistent initial states, unreachable actions, vacuous invariants, duplicate/subsumed properties, and invariants preserved only because no progress is possible.
  ...
  - [x] Detect actions unreachable from the transition system's initial states within explicit bounded, finite-state-complete, inductive, or proven-strengthening proof modes; retain general infinite-state cases as explicitly bounded/inconclusive results.
    ...
    - [ ] Synthesize general affine/polyhedral, quantified, and richer field-correlated collection invariants.
      ...
      - [x] Accept state-backed and otherwise normalizable pure `throw` expressions through the same completion lattice when catch does not inspect the payload; validate evaluation purity without claiming thrown-value refinement, and keep effectful expressions fail-closed.
