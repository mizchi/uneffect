...
- [ ] Extend frontend parity from declarations to inferred effects, call edges, ordered events, and real Corsa checker facts. ([#8](https://github.com/mizchi/uneffect/issues/8))
  ...
  - [ ] Route the main analyzer's global `Fetch`/`Console` catalog resolution through the compiler-neutral semantic sidecar, retaining TypeScript AST traversal and dual-frontend parity in CI before making Corsa authoritative.
  - [ ] Require stable Corsa APIs for aliased symbols, resolved signatures/property identity, and batch symbol queries before migrating named or re-exported Node builtin effects.
