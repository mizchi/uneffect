...
`).map((task) => task.text)).toEqual(["stale"]);
  ...
  it("keeps the active issue index in roadmap execution order", () => {
    ...
    expect(rows.filter(([status]) => status === "Active")).toEqual([["Active", 3, 53]]);
  });
  ...
  it("keeps one ordered immediate queue with explicit handoff conditions", () => {
    ...
    expect(rows).toEqual([
      [
        1,
        53,
        "Workhub-shaped plain/asserted Promise call returns reach parity; conditional/non-call/nested-wrapper controls, tamper rejection, benchmark, docs, full checks, and remote CI pass.",
      ],
    ]);
