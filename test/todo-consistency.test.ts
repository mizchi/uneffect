...
`).map((task) => task.text)).toEqual(["stale"]);
  ...
  it("lists every open issue under its owning roadmap phase", () => {
    ...
    for (const issue of [25, 37, 38, 39, 40, 41, 47, 48, 2, 5, 42, 43, 4, 6]) {
  ...
  it("keeps the active issue index in roadmap execution order", () => {
    ...
    expect(rows.filter(([status]) => status === "Active")).toEqual([["Active", 3, 52]]);
  });
  ...
  it("keeps one ordered immediate queue with explicit handoff conditions", () => {
    ...
    expect(rows).toEqual([
      [
        1,
        52,
        "Workhub-shaped direct awaits under one `if` branch reach parity; opposite polarity, nested/loop controls, evidence tampering, benchmark, docs, full checks, and remote CI are covered.",
      ],
    ]);
