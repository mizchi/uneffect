...
`).map((task) => task.text)).toEqual(["stale"]);
  ...
  it("keeps the active issue index in roadmap execution order", () => {
    ...
    expect(rows.filter(([status]) => status === "Active")).toEqual([["Active", 3, 54]]);
  });
  ...
  it("keeps one ordered immediate queue with explicit handoff conditions", () => {
    ...
    expect(rows).toEqual([
      [
        1,
        54,
        "Workhub-shaped directory/append fs atoms reach parity; local/local-module controls, metadata tampering, benchmark, docs, full checks, and remote CI pass.",
      ],
    ]);
