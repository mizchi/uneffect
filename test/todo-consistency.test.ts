...
`).map((task) => task.text)).toEqual(["stale"]);
  ...
  it("keeps every open issue visible in the active index and boundary docs", () => {
    ...
    const roadmap = readFileSync("docs/roadmap.md", "utf8");
    const issueNumbers = [2, 4, 5, 6, 7, 8, 10, 13, 16, 18, 24, 25, 44];
  ...
  it("keeps one ordered immediate queue with explicit handoff conditions", () => {
    ...
    expect(rows).toEqual([
      [1, 44, "Prove every direct scalar Map key against one unambiguous immutable literal Set and reject the whole universe if any proof fails."],
    ]);
