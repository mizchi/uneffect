...
describe("Uneffect dogfood", () => {
  ...
  it("proves a loop-invariant piecewise recurrence through a CFG diamond", async () => {
    ...
    expect(analysis.obligations).toContainEqual(expect.objectContaining({
      ...
      status: "verified",
      controlJoins: [expect.objectContaining({
        order: 0, selector: { kind: "boolean-state", state: "sampled" }, rule: "predicate-correlated-affine-phi",
      })],
      recurrenceProof: expect.objectContaining({ status: "verified" }),
    }));
  });
  ...
  it("preserves a nested Boolean resource decision through shared recovery", () => {
  ...
  it("preserves non-uniform return through branch cleanup and mandatory finally", () => {
    ...
    expect(resources.mirror?.controlPaths[0]).toContainEqual(expect.objectContaining({
      expected: false,
      role: "fallthrough",
    }));
    expect(result.returnCompletions.filter(({ owner }) => owner === "deliverNonUniform")).toHaveLength(1);
    expect(result.diagnostics).not.toContainEqual(expect.objectContaining({ kind: "floating-promise" }));
  ...
  it("constructs a finite property domain from the real Datadog metric-name validator", () => {
    ...
    expect(result.boundaries).toContainEqual(expect.objectContaining({
      functionName: "datadogMetricKey",
      generators: [{
        kind: "specialized",
        predicate: "isDatadogMetricName",
        values: ["bad space", "requests.total", "a"],
      }],
    }));
