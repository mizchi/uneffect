...
const manifest = (fileName: string, invariant = false) => refinementManifest(
  fileName,
  "counter",
  { increment: "increment" },
  {
    create: "createCounter",
    observe: "observeCounter",
    invariants: invariant ? { guarded: "guarded" } : {},
  },
);
...
describe("Z3-backed refinement expression equivalence", () => {
  ...
  it("proves integer update equivalence and discharges syntax-only action mismatches", async () => {
    ...
    expect(await validateRefinementActionBodiesWithZ3(
      "integer-equivalence.ts", source, "counter", spec,
      refinementManifest("integer-equivalence.ts", "counter", { increment: "increment" }),
    )).toEqual([]);
  });
  ...
  it("accepts logically equivalent invariant and guard syntax", async () => {
    ...
    expect(await validateRefinementActionBodiesWithZ3("counter.ts", source, "counter", spec, manifest("counter.ts", true))).toEqual([]);
    expect(await validateRefinementInvariantBodiesWithZ3("counter.ts", source, "counter", spec, manifest("counter.ts", true))).toEqual([]);
  });
  ...
  it("does not turn unsupported implementation bodies into solver claims", async () => {
    ...
    `;
    const spec = parseSpec("counter.ts", source).temporal;
    expect(await validateRefinementActionBodiesWithZ3("counter.ts", source, "counter", spec, manifest("counter.ts", true))).toContainEqual(
