...
function leaseModel(skewGrace: number): string {
  return `
    /* uneffect: clock realNow: 1 */ /* uneffect: state leaseExpiryA: int */ /* uneffect: state localDeadlineA: int */ /* uneffect: state ownerEpoch: int */ /* uneffect: state residentEpochA: int */ /* uneffect: state residentEpochB: int */ /* uneffect: state ownerIsA: bool */ /* uneffect: init leaseExpiryA = 10 */ /* uneffect: init localDeadlineA = 10 */ /* uneffect: init ownerEpoch = 1 */ /* uneffect: init residentEpochA = 1 */ /* uneffect: init residentEpochB = 0 */ /* uneffect: init ownerIsA = true */ /* uneffect: action takeoverB: ownerIsA' = false, ownerEpoch' = ownerEpoch + 1 */ /* uneffect: action_when takeoverB: ownerIsA && realNow + 1 >= leaseExpiryA + ${skewGrace} */ /* uneffect: action publishB: residentEpochB' = ownerEpoch */ /* uneffect: action_when publishB: !ownerIsA && residentEpochB !== ownerEpoch */ /* uneffect:always singleWriter: !(residentEpochA > 0 && realNow < localDeadlineA && residentEpochB > 0) */
  `;
}

function collectionLeaseModel(skewGrace: number): string {
  return `/* uneffect: clock realNow: 1 */ /* uneffect: state nodes: Set<int> */ /* uneffect: state activeWriters: Set<int> */ /* uneffect: state residentEpochs: Map<int, int> */ /* uneffect: state lease: { owner: int, epoch: int } */ /* uneffect: init nodes = Set(1, 2) */ /* uneffect: init activeWriters = Set(1) */ /* uneffect: init residentEpochs = Map([[1, 1], [2, 0]]) */ /* uneffect: init lease = { owner: 1, epoch: 1 } */ /* uneffect: action takeover: lease' = { ...lease, owner: 2, epoch: lease.epoch + 1 } */ /* uneffect: action_when takeover: lease.owner === 1 && realNow + 1 >= 10 + ${skewGrace} */ /* uneffect: action publish: activeWriters' = activeWriters.union(Set(2)), residentEpochs' = residentEpochs.put(2, 2) */ /* uneffect: action_when publish: lease.owner === 2 && !activeWriters.contains(2) */ /* uneffect:always writersAreNodes: activeWriters.forall(node => nodes.contains(node)) */ /* uneffect:always epochsAreNonNegative: residentEpochs.values().forall(epoch => epoch >= 0) */ /* uneffect:always singleWriter: !(activeWriters.contains(1) && realNow < 10 && activeWriters.contains(2)) */`;
}

function capabilityRequestModel(enforceAuthority: boolean): string {
  return `/* uneffect: state authority: { owners: Map<int, int>, allowedResources: Set<int>, allowedOwners: Set<int> } */ /* uneffect: state auditArmed: bool */ /* uneffect: init authority = { owners: Map([[1, 10]]), allowedResources: Set(1, 2), allowedOwners: Set(10, 20) } */ /* uneffect: init auditArmed = false */ /* uneffect: action requestWrite: authority' = { ...authority, owners: authority.owners.put(2, 20) } */ /* uneffect: ${enforceAuthority ? "action_when requestWrite: authority.allowedResources.contains(2) && authority.allowedOwners.contains(20)" : ""} */ /* uneffect: action armAudit: auditArmed' = true */ /* uneffect: action observeEscalation: auditArmed' = auditArmed */ /* uneffect: action_when observeEscalation: auditArmed && authority.owners.keys().contains(2) && !authority.allowedResources.contains(2) */ /* uneffect: action observeOwnerEscalation: auditArmed' = auditArmed */ /* uneffect: action_when observeOwnerEscalation: auditArmed && authority.owners.values().contains(20) && !authority.allowedOwners.contains(20) */ /* uneffect:always requestWithinAuthority: authority.owners.keys().forall(resource => authority.allowedResources.contains(resource)) */ /* uneffect:always ownerWithinAuthority: authority.owners.values().forall(owner => authority.allowedOwners.contains(owner)) */`;
}
...
function leaseLifecycleModel(fencedCommit: boolean): string {
  return `/* uneffect: clock realNow: 1 */ /* uneffect: state leaseExpiry: int */ /* uneffect: state ownerEpoch: int */ /* uneffect: state workerAlive: bool */ /* uneffect: state renewalInFlight: bool */ /* uneffect: state renewalEpoch: int */ /* uneffect: state selfFenced: bool */ /* uneffect: state resourceHeld: bool */ /* uneffect: state writeInFlight: bool */ /* uneffect: state writeEpoch: int */ /* uneffect: state badCommit: bool */ /* uneffect: init leaseExpiry = 2 */ /* uneffect: init ownerEpoch = 1 */ /* uneffect: init workerAlive = true */ /* uneffect: init renewalInFlight = false */ /* uneffect: init renewalEpoch = 0 */ /* uneffect: init selfFenced = false */ /* uneffect: init resourceHeld = true */ /* uneffect: init writeInFlight = false */ /* uneffect: init writeEpoch = 0 */ /* uneffect: init badCommit = false */ /* uneffect: action startRenewal: renewalInFlight' = true, renewalEpoch' = ownerEpoch */ /* uneffect: action_when startRenewal: workerAlive && !selfFenced && !renewalInFlight */ /* uneffect: action completeRenewal: renewalInFlight' = false, leaseExpiry' = realNow + 2 */ /* uneffect: action_when completeRenewal: renewalInFlight && workerAlive && !selfFenced && renewalEpoch === ownerEpoch */ /* uneffect: action renewalCasFailure: renewalInFlight' = false, selfFenced' = true */ /* uneffect: action_when renewalCasFailure: renewalInFlight */ /* uneffect: action startWrite: writeInFlight' = true, writeEpoch' = ownerEpoch */ /* uneffect: action_when startWrite: workerAlive && !selfFenced && !writeInFlight */ /* uneffect: action takeover: ownerEpoch' = ownerEpoch + 1 */ /* uneffect: action_when takeover: realNow >= leaseExpiry */ /* uneffect: action normalizeInvalidEpoch: ownerEpoch' = 0 */ /* uneffect: action_when normalizeInvalidEpoch: ownerEpoch < 0 */ /* uneffect: action observeZeroEpoch: badCommit' = true */ /* uneffect: action_when observeZeroEpoch: ownerEpoch === 0 */ /* uneffect: action completeWrite: writeInFlight' = false, badCommit' = writeEpoch !== ownerEpoch */ /* uneffect: action_when completeWrite: writeInFlight${fencedCommit ? " && writeEpoch === ownerEpoch" : ""} */ /* uneffect: action crash: workerAlive' = false */ /* uneffect: action_when crash: workerAlive */ /* uneffect: action gc: resourceHeld' = false */ /* uneffect: action_when gc: !workerAlive && resourceHeld */ /* uneffect:always noStaleCommit: !badCommit */ /* uneffect:always ownerEpochPositive: ownerEpoch > 0 */ /* uneffect:always casFailureFences: !selfFenced || !renewalInFlight */ /* uneffect:always gcDoesNotInventResources: resourceHeld || !workerAlive */`;
}
...
describe("Node Lease clock-skew model", () => {
  it("extracts record-valued lease grants across Quint, Z3, TLC, and replay", async () => {
    ...
    const dynamic = parseSpec("dynamic-record-grant.ts", `/* uneffect: state candidate: { owner: string, epoch: int, valid: bool } */ /* uneffect: state grants: Set<{ owner: string, epoch: int, valid: bool }> */ /* uneffect: init candidate = { owner: "node-a", epoch: 1, valid: true } */ /* uneffect: init grants = Set(candidate) */ /* uneffect: action retain: grants' = grants */ /* uneffect:always validGrantHasEpoch: grants.forall(grant => !grant.valid || grant.epoch > 0) */`).temporal;
  ...
  it("preserves production string node identities across Quint, Z3, TLC trace import, and replay", async () => {
    ...
    const unsupportedControlCharacter = parseSpec("string-control.ts", `/* uneffect: state id: string */ /* uneffect: init id = "\\n" */ /* uneffect: action retain: id' = id */ /* uneffect:always stable: id === "\\n" */`).temporal;
  ...
  it("rules out worker-resource starvation only under the declared weak fairness", async () => {
    const model = (fair: boolean) => parseSpec("lease-gc-liveness.ts", `/* uneffect: state workerAlive: bool */ /* uneffect: state resourceHeld: bool */ /* uneffect: init workerAlive = true */ /* uneffect: init resourceHeld = true */ /* uneffect: action idle: resourceHeld' = resourceHeld */ /* uneffect: action crash: workerAlive' = false */ /* uneffect: action_when crash: workerAlive */ /* uneffect: action gc: resourceHeld' = false */ /* uneffect: action_when gc: !workerAlive && resourceHeld */ /* uneffect: ${fair ? "action_fair crash: weak" : ""} */ /* uneffect: ${fair ? "action_fair gc: weak" : ""} */ /* uneffect: eventually resourceReleased: !resourceHeld */`).temporal;
