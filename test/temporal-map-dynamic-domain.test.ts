...
const fixture = `/* uneffect:
  ...
  temporal selectedIsNode: nodes.contains(selected)
  temporal selectedHasLease: leases.getOrElse(selected, 0) > 0
*/`;
...
describe("proved finite domains for dynamic temporal Map keys", () => {
  ...
  it("fails closed when any finite-domain proof premise is absent", async () => {
    for (const [name, source] of [
      ["missing-property", fixture.replace("  temporal selectedIsNode: nodes.contains(selected)\n", "")],
      ...
      ["ambiguous-domain", fixture.replace(
        ...
        "temporal selectedIsNode: nodes.contains(selected)",
        "temporal selectedIsNode: nodes.contains(selected)\n  temporal selectedIsOtherNode: otherNodes.contains(selected)",
      )],
    ] as const) {
  ...
  it("supports the same proved fragment for boolean Map keys", async () => {
    const temporal = parseSpec("dynamic-bool-map-domain.ts", `/* uneffect:
      state flags: Set<bool>
      state selected: bool
      state values: Map<bool, int>
      init flags = Set(false, true)
      init selected = false
      init values = Map([[false, 1]])
      action selectMissing: selected' = true
      temporal selectedIsFlag: flags.contains(selected)
      temporal selectedHasValue: values.getOrElse(selected, 0) > 0
    */`).temporal;
