...
`, spec, "tlc-model-sha256");
  ...
  it("normalizes finite Set, Map, and record TLC values and recovers collection actions", () => {
    const spec = parseSpec("collections.ts", `/* uneffect: state writers: Set<int> */ /* uneffect: state epochs: Map<int, int> */ /* uneffect: state lease: { owner: int, valid: bool } */ /* uneffect: init writers = Set(1) */ /* uneffect: init epochs = Map([[1, 1]]) */ /* uneffect: init lease = { owner: 1, valid: true } */ /* uneffect: action publish: writers' = writers.union(Set(2)), epochs' = epochs.put(2, 1), lease' = { ...lease, owner: 2 } */ /* uneffect:always oneWriter: writers.size() <= 1 */`).temporal;
    const trace = parseTlcCounterexample(`
Error: Invariant q_inv is violated.
State 1: <Initial predicate>
/\\ writers = {1}
...
`, spec, "collections-model");
  ...
  it("rejects non-violations and ambiguous TLC action recovery", () => {
    const spec = parseSpec("ambiguous.ts", `/* uneffect: state value: int */ /* uneffect: init value = 0 */ /* uneffect: action first: value' = value + 1 */ /* uneffect: action second: value' = value + 1 */ /* uneffect:always zero: value === 0 */`).temporal;
