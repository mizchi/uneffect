...
export function lowerNativeReturnPaths(body: FunctionBody, expression: (node: Expression) => NativeScalar): NativeReturnPath[] {
  const blocks: BasicBlock<Paths>[] = [];
  const returns: Array<{ id: string; span: { start: number; end: number }; result: NativeScalar }> = [];
  ...
  const statement = (node: Statement, next: string): string => {
    ...
    if (node.type === "ReturnStatement") {
      ...
      const result = expression(node.argument);
      const id = block([], () => []);
      returns.push({ id, span: { start: node.start, end: node.end }, result });
      return id;
    }
    if (node.type === "IfStatement") {
      ...
      return block([...new Set([yes, no])], paths => condition.kind === "boolean"
  ...
  const paths = returns.sort((left, right) => left.span.start - right.span.start).flatMap(({ id, span, result }) =>
    [...(solved.states.get(id)?.values() ?? [])].map(conditions => ({ span, result, conditions })));
