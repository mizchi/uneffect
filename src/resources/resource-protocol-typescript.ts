...
import { externalContractForCall, type ExternalFunctionEffectContract } from "../effects/effects.js";
...
import { interpretBuiltinCallSemantics, type ProjectedValue } from "../effects/builtin-semantic-interpreter.js";
import { resolveStableCallableSymbol } from "../frontends/typescript/stable-callable.js";
import { builtinContractRegistry, type BuiltinContractRegistry } from "../effects/builtin-contracts.js";
...
export function collectCallableExceptionalTransitionSites(
  ...
  const visit = (node: ts.Node): void => {
    ...
    if (ts.isCallExpression(node)) {
      ...
      const symbol = resolveStableCallableSymbol(checker, node.expression)
        ?? (candidate ? resolvedSymbol(checker, node.expression) : undefined);
      const declaration = symbol?.valueDeclaration ?? symbol?.declarations?.[0];
